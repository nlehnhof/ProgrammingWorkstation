...
drop_output1 = tk.Text(drop_frame1, height=10, width=25, state='disabled')
...
excel_files = get_excel_files("C:/Users/u317029/Documents/teltonika")
print(excel_files)
...
drop_output = tk.Text(drop_frame, height=10, width=25, state='disabled')
...
excel_output = tk.Text(excel_frame, height=10, width=40, state='disabled')
...
def lookup_excel(sheet):
    ...
    try:
        ...
        show_excel(f"Error Reading File")
...
status_text = tk.Text(check_frame, height=10, width=40, state='disabled')
...
def run_main_script(excel, gate, temp):
                    ...
                    for row in sheet.iter_rows(values_only=False):
                        for cell in row:
                            if str(cell.value) == to_find:
                                ...
                                t_row = cell.row
                                t_col = cell.column
                                file_path_crash = os.path.abspath(f"C:/Users/u317029/Documents/teltonika/crash_logs/{filename_crash}") 
                    ...
                    for row in sheet.iter_rows(values_only=False):
                        for cell in row:
                            if str(cell.value) == to_find:
                                t_row = cell.row
                                t_col = cell.column
                                file_path_label = os.path.abspath(f"C:/Users/u317029/Documents/teltonika/router_labels/{filename_label}") 
...
log_output = tk.Text(log_frame, height=10, width=30, state='disabled')
...
test_output = tk.Text(test_frame, height=10, width=30, state='disabled')
...
def run_test_script():
    ...
    with open("C:/Users/u317029/Documents/teltonika/testfile/FloodLighToggle.py", 'r') as file:
        ...
        if new_ip in verify:
            ...
            log("Incorrect! New Gate IP Not Updated")
            test_error = True
...
info_output0 = tk.Text(info_frame0, height=8, width=25, state='disabled')
...
info_output1 = tk.Text(info_frame1, height=8, width=25, state='disabled')
...
bl1_output = tk.Text(bl1_frame, height=8, width=40, state='disabled')
...
bl2_output = tk.Text(bl2_frame, height=8, width=40, state='disabled')
...
bl3_output = tk.Text(bl3_frame, height=8, width=30, state='disabled')
...
bl4_output = tk.Text(bl4_frame, height=8, width=30, state='disabled')
...
user_entry = tk.Entry(input_frame, width=20)
