...
def run_main_script(airport, gate, temp_pass, device):
    print("Temp pass: ", temp_pass, flush=True)
    ...
    excel = os.path.join(device_path, airport + ".xlsx")
    ...
    with process.stdout: # type: ignore
        ...
        if "Incorrect" in statement:
            ...
            try:
                ...
                wb.save(excel)
                filepath = os.path.join(device_path, filepath)
            ...
            #OSHKOSH LOG
            oshkosh_log_path = os.path.join(device_path, "oshkosh_log.xlsx")
    ...
    if traceback is True:
        try:
            ...
            timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            crash_folder = os.path.join(device_path, "crash_logs")
            os.makedirs(crash_folder, exist_ok=True)
            excel_name = excel.removesuffix(".xlsx")
    ...
    try:
        print("Creating Label")            
        timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        router_labels = os.path.join(device_path, "router_labels")
        os.makedirs(router_labels, exist_ok=True)
        excel_name = airport
    ...
    run_test_script(device, gate, excel)
    print("Workflow finished.", flush=True)
    
################################################ TEST SCRIPT ##############################################
def run_test_script(device: str, gate: str, excel: str):
    ...
    currdir = os.path.join(current_dir, f"devices/{device}")
    ...
    time.sleep(s_pause)
    ssh_bbb_upload(os.path.join(currdir, "testfile/FloodLighToggle.py"), "/home/raj/FloodLighToggle.py")
    time.sleep(s_pause)
    ...
    if "Bytes in received" not in output:
        ...
        timestamp = datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
        crash_folder = os.path.join(currdir, "crash_logs")
        os.makedirs(crash_folder, exist_ok=True)

        filename_crash = f"test_crash_log_{timestamp}.txt"
        filepath = os.path.join(crash_folder, filename_crash)
            ...
            for row in sheet.iter_rows(values_only=False):
                for cell in row:
                    if str(cell.value) == to_find:
                        ...
                        t_row = cell.row
                        t_col = cell.column
                        file_path_crash = os.path.abspath(os.path.join(currdir, f"crash_logs/{filename_crash}")) 
    ...
    oshkosh_log_path = os.path.join(currdir, "oshkosh_log.xlsx")
